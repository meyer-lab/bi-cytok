...
def load_existing_cite_data():
    ...
    try:
        ...
        print(
            f"Columns: {list(cite_df.columns[:10])}{'...' if len(cite_df.columns) > 10 else ''}"
        )
...
def load_adt_data():
    ...
    try:
        ...
        adata_adt = sc.read_10x_mtx(
            ADT_DATA_PATH, prefix="GSM5008738_ADT_3P-", cache=False
        )
...
def load_rna_data():
    ...
    try:
        ...
        adata_rna = sc.read_10x_mtx(
            RNA_DATA_PATH, prefix="GSM5008737_RNA_3P-", cache=False
        )
...
def main():
    ...
    if barcode_comparison and adt_data and rna_data:
        print("\nBarcode consistency:")
        print(
            f"- ADT and RNA share {len(barcode_comparison['common_barcodes']):,} common barcodes"
        )
        if annot_df is not None:
            # Check if annotation barcodes match RNA/ADT barcodes
            annot_barcodes = set(annot_df["barcode"].values)
            rna_vs_annot = barcode_comparison["rna_barcodes"].intersection(
                annot_barcodes
            )
...
def compare_cell_type_labels():
    ...
    try:
        ...
        if cite_df.shape[0] == hao_meta.shape[0]:
            ...
            print(
                f"✗ Cell counts differ: CITE={cite_df.shape[0]:,}, Hao={hao_meta.shape[0]:,}"
            )
        ...
        print("Level 1 (CellType1 vs celltype.l1):")
...
def compare_adt_expression_values():
    ...
    try:
        ...
        adt_data = sc.read_10x_mtx(adt_dir, prefix="GSM5008738_ADT_3P-", cache=False)
        ...
        print(
            f"  Coverage: {len(common_markers) / len(cite_markers) * 100:.1f}% of CITE markers"
        )
        ...
        if len(common_markers) > 0:
            ...
            for marker in sample_markers:
                if marker in cite_df.columns:
                    ...
                    print(f"\n{marker}:")
                    print(
                        f"  CITE - Range: [{cite_values.min():.2f}, {cite_values.max():.2f}], "
                        f"Mean: {cite_values.mean():.2f}, Std: {cite_values.std():.2f}"
                    )
                    print(
                        f"  Hao  - Range: [{hao_values.min():.2f}, {hao_values.max():.2f}], "
                    ...
                    if len(cite_values) >= 1000 and len(hao_values) >= 1000:
                        correlation = pd.Series(cite_values[:1000]).corr(
                            pd.Series(hao_values[:1000])
                        )
            ...
            if all_correlations:
                ...
                if mean_correlation > 0.95:
                    print(
                        "  ✓ IDENTICAL: Expression values are highly correlated (likely same processing)"
                    )
                elif mean_correlation > 0.8:
                    print(
                        "  ~ SIMILAR: Expression values are well correlated (possibly different normalization)"
                    )
                elif mean_correlation > 0.5:
                    print(
                        "  ? MODERATE: Expression values are moderately correlated (different processing)"
                    )
                else:
                    print(
                        "  ✗ DIFFERENT: Expression values are poorly correlated (different data/processing)"
                    )
