...
def scan_KL_EMD(
    ...
    output_shape = (rec_abundances.shape[0]) * dim + (len(targ_cell_types))
    ...
    for i, cell_type in enumerate(targ_cell_types):
        ...
        sampled_cell_type_labels = sampled_abun_DF["Cell Type"].to_numpy()
...
def scan_selectivity(
    ...
    opt_affs_scan = np.full(output_shape, np.nan)
    ...
    for i, cell_type in enumerate(targ_cell_types):
        ...
        print(sum(targ_mask))
        off_targ_mask = ~targ_mask
        print(sum(off_targ_mask))

        if signal_col is not None:
            signal_rec_abun = np.reshape(sampled_rec_abundances[:, signal_col], (-1, 1))

        if dim == 1:
            opt_selecs = np.full((sampled_rec_abundances.shape[0]), np.nan)
            if signal_col is not None:
                opt_affs = np.full((sampled_rec_abundances.shape[0], dim + 1), np.nan)
            else:
                opt_affs = np.full((sampled_rec_abundances.shape[0], dim), np.nan)
            opt_Kx_stars = np.full((sampled_rec_abundances.shape[0]), np.nan)

            for j in range(sampled_rec_abundances.shape[1]):
                rec_abun_pruned = np.reshape(sampled_rec_abundances[:, j], (-1, 1))
                print(rec_abun_pruned.shape)
                print(signal_rec_abun.shape)
        ...
        if dim == 2:
            ...
            # Triangular indices assumes symmetry across the diagonal which is not
            #   true if there is no designated signal recpetor or if valencies are
            #   asymmetric
