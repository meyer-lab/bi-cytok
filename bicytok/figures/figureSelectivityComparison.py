...
- Shows selectivity values, optimal affinities, and binding patterns across scaling
  factors
"""
...
def min_off_targ_selec_weighted_off_target(
    ...
    """
    Weighted selectivity using off-target/target ratio - weighted by receptor
      abundance.
    """
    assert targRecs.shape[1] == offTargRecs.shape[1]
    assert targRecs.shape[1] >= 2
...
def makeFigure():
    ...
    # Plot 6: Selectivity ratio comparison
    #   (normalized to signal-only at conversion factor = 1)
    baseline_idx = np.argmin(np.abs(conversion_factors - 1.0))
    signal_only_baseline = results["Signal Only (Full/Target)"]["selectivity"][
        baseline_idx
    ]
