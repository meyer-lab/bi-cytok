...
def makeFigure():
    ...
    targCell = 'Treg'
    cells = ['Treg', 'CD8 Naive', 'NK', 'CD8 TEM', 'CD4 Naive', 'CD4 CTL', 'CD8 TCM', 'CD4 TEM', 'NK Proliferating',
    'NK_CD56bright']
    ...
    bindings = get_cell_bindings(epitopesDF, secondary, ['CD25', epitope], affs, 0.1, [valency, valency, valency])
    bindings['Percent Bound of Signal Receptor'] = (bindings['Receptor Bound'] / bindings[secondary]) * 10

    palette = sns.color_palette("husl", 10)
    sns.barplot(data=bindings, x=bindings.index, y='Receptor Bound', palette=palette, ax=ax[0])
    sns.barplot(data=bindings, x=bindings.index, y='Percent Bound of Signal Receptor', palette=palette, ax=ax[1])
