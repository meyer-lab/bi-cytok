...
def common_code(weightDF, dataset, signal_receptor, target_cells):
    ...
    for column in dataset.columns:
        if column != signal_receptor and column not in [
            "CellType1",
            "CellType2",
            "CellType3",
        ]:
    ...
    target_cells_df = dataset[
        ...
        | (dataset["CellType1"] == target_cells)
    ]
...
def EMD_2D(dataset, signal_receptor, target_cells, ax):
    ...
    (
        ...
        non_signal_receptors,
        conversion_factor_sig,
    ) = common_code(weightDF, filtered_dataset, signal_receptor, target_cells)
    results = []
...
def EMD_3D(dataset1, target_cells, ax=None):
    ...
    receptor_names = [
        ...
        and np.mean(target_cells_df[col]) > np.mean(off_target_cells_df[col])
        and np.mean(target_cells_df[col]) > 5
    ]
    ...
    for receptor1_name in receptor_names:
        for receptor2_name in receptor_names:
            for receptor3_name in receptor_names:
                if (
                    receptor1_name != receptor2_name
                    and receptor1_name != receptor3_name
                    and receptor2_name != receptor3_name
                ):
                    # Get on and off-target counts for receptor1 2 and 3
                    receptor1_on_target_counts = target_cells_df[receptor1_name].values
                    receptor1_off_target_counts = off_target_cells_df[
                        receptor1_name
                    ].values
                    receptor2_on_target_counts = target_cells_df[receptor2_name].values
                    receptor2_off_target_counts = off_target_cells_df[
                        receptor2_name
                    ].values
                    receptor3_on_target_counts = target_cells_df[receptor3_name].values
                    receptor3_off_target_counts = off_target_cells_df[
                        receptor3_name
                    ].values
                    conversion_factor_receptor1 = get_conversion_factor(
                        weightDF, receptor1_name
                    )
                    conversion_factor_receptor2 = get_conversion_factor(
                        weightDF, receptor2_name
                    )
                    conversion_factor_receptor3 = get_conversion_factor(
                        weightDF, receptor3_name
                    )
                    # Apply the conversion factors to the counts
                    receptor1_on_target_counts = (
                        receptor1_on_target_counts * conversion_factor_receptor1
                    )
                    receptor1_off_target_counts = (
                        receptor1_off_target_counts * conversion_factor_receptor1
                    )
                    receptor2_on_target_counts = (
                        receptor2_on_target_counts * conversion_factor_receptor2
                    )
                    receptor2_off_target_counts = (
                        receptor2_off_target_counts * conversion_factor_receptor2
                    )
                    receptor3_on_target_counts = (
                        receptor3_on_target_counts * conversion_factor_receptor3
                    )
                    receptor3_off_target_counts = (
                        receptor3_off_target_counts * conversion_factor_receptor3
                    )
                    ...
                    average_receptor_counts_1 = np.mean(
                        np.concatenate(
                            (receptor1_on_target_counts, receptor1_off_target_counts)
                        ),
                        axis=0,
                    )
                    average_receptor_counts_2 = np.mean(
                        np.concatenate(
                            (receptor2_on_target_counts, receptor2_off_target_counts)
                        ),
                        axis=0,
                    )
                    average_receptor_counts_3 = np.mean(
                        np.concatenate(
                            (receptor3_on_target_counts, receptor3_off_target_counts)
                        ),
                        axis=0,
                    )
                    if (
                        average_receptor_counts_1_on > 5
                        and average_receptor_counts_2_on > 5
                        and average_receptor_counts_3_on > 5
                        and average_receptor_counts_1_on > average_receptor_counts_1_off
                        and average_receptor_counts_2_on > average_receptor_counts_2_off
                        and average_receptor_counts_3_on > average_receptor_counts_3_off
                    ):
                        receptor1_on_target_counts = (
                            receptor1_on_target_counts.astype(float)
                            / average_receptor_counts_1
                        )
                        receptor1_off_target_counts = (
                            receptor1_off_target_counts.astype(float)
                            / average_receptor_counts_1
                        )
                        receptor2_on_target_counts = (
                            receptor2_on_target_counts.astype(float)
                            / average_receptor_counts_2
                        )
                        receptor2_off_target_counts = (
                            receptor2_off_target_counts.astype(float)
                            / average_receptor_counts_2
                        )
                        receptor3_on_target_counts = (
                            receptor3_on_target_counts.astype(float)
                        ...
                        on_target_counts = np.concatenate(
                            (
                                ...
                                receptor2_on_target_counts[:, np.newaxis],
                                receptor3_on_target_counts[:, np.newaxis],
                            ),
                            axis=1,
                        )
                        off_target_counts = np.concatenate(
                            (
                                receptor1_off_target_counts[:, np.newaxis],
                                receptor2_off_target_counts[:, np.newaxis],
                                receptor3_off_target_counts[:, np.newaxis],
                            ),
                            axis=1,
                        )
                        average_receptor_counts = np.mean(
                            np.concatenate((on_target_counts, off_target_counts)),
                            axis=0,
                        )
                        on_target_counts = (
                            on_target_counts.astype(float) / average_receptor_counts
                        )
                        off_target_counts = (
                            off_target_counts.astype(float) / average_receptor_counts
                        )
                        M = ot.dist(on_target_counts, off_target_counts)
                        a = (
                            np.ones(on_target_counts.shape[0])
                            / on_target_counts.shape[0]
                        )
                        b = (
                            np.ones(off_target_counts.shape[0])
                            / off_target_counts.shape[0]
                        )
                        ...
                        results.append(
                            (0, receptor1_name, receptor2_name, receptor3_name)
                        )

    sorted_results = sorted(results, reverse=True)
    top_receptor_info = [
        (receptor1_name, receptor2_name, receptor3_name, optimal_transport)
        for optimal_transport, receptor1_name, receptor2_name, receptor3_name in sorted_results[
            :10
        ]
    ]
    ...
    if ax is not None:
        ...
        ax.set_title(
            f"Top 10 Receptor Pair Distances (3D) for {target_cells}", fontsize=14
        )
        ax.set_xticks(range(len(receptor_pairs)))
        ax.set_xticklabels(
            [f"{pair[0]} - {pair[1]} - {pair[2]}" for pair in receptor_pairs],
            rotation="vertical",
            fontsize=14,
        )
    return sorted_results
...
def EMD_2D_pair(dataset, target_cells, signal_receptor, special_receptor):
    ...
    off_target_receptor_counts = off_target_cells_df[
        [signal_receptor, special_receptor]
    ].values
...
def KL_divergence_2D_pair(dataset, target_cells, signal_receptor, special_receptor):
    ...
    KL_div = calculate_kl_divergence_2D(
        target_receptor_counts[:, 1], off_target_receptor_counts[:, 1]
    )
...
def EMD_1D(dataset, target_cells, ax):
    ...
    for receptor_name in receptorsdf:
        ...
        target_receptor_counts = (
            target_receptor_counts.astype(float) * conversion_factor
        )
        off_target_receptor_counts = (
            off_target_receptor_counts.astype(float) * conversion_factor
        )
    ...
    sorted_results = sorted(results, reverse=True)
    top_receptor_info = [
        (receptor_name, optimal_transport)
        for optimal_transport, receptor_name in sorted_results[:5]
    ]
    # bar graph
    ...
    ax.set_xticklabels(receptor_names, rotation="vertical")
    print(
        "The 5 receptors which achieve the greatest positive distance from target-off-target cells are:",
        top_receptor_info,
    )
    return sorted_results
...
def EMD1Dvs2D_Analysis(
    receptor_names, target_cells, signal_receptor, dataset, ax1, ax2, ax3, ax4
):
    filtered_data_1D = []
    filtered_data_2D = []
    ...
    cell_types = (
        set(dataset["CellType1"])
        .union(dataset["CellType2"])
        .union(dataset["CellType3"])
    )
    offtarg_cell_types = [
        cell_type for cell_type in cell_types if cell_type != target_cells
    ]
    epitopes = [
        column
        for column in dataset.columns
    ...
    ax3.scatter(
        data_1D_distances, selectivity_distances, color="blue", label="filtered_data_1D"
    )
    for x, y, name in zip(data_1D_distances, selectivity_distances, data_names):
        ax3.text(x, y, name, fontsize=8, ha="left", va="top")

    ax4.scatter(
        data_2D_distances, selectivity_distances, color="red", label="filtered_data_2D"
    )
...
def KL_divergence_2D(dataset, signal_receptor, target_cells, ax):
    ...
    (
        ...
        non_signal_receptors,
        conversion_factor_sig,
    ) = common_code(weightDF, dataset, signal_receptor, target_cells)
    results = []
    for receptor_name in non_signal_receptors:
        target_receptor_counts = target_cells_df[
            [signal_receptor, receptor_name]
...
def plot_kl_divergence_curves(
    dataset, signal_receptor, special_receptor, target_cells, ax
):
    CITE_DF = importCITE()
    ...
    target_receptor_counts = (
        target_cells_df[special_receptor].values * conversion_factor_sig
    )
    off_target_receptor_counts = (
        off_target_cells_df[special_receptor].values * conversion_factor_sig
    )

    avg_special_receptor = np.mean(
        np.concatenate([target_receptor_counts, off_target_receptor_counts])
    )
    ...
    x_vals = np.linspace(
        min(target_receptor_counts.min(), off_target_receptor_counts.min()),
        max(target_receptor_counts.max(), off_target_receptor_counts.max()),
        1000,
    )
    ...
    KL_div = calculate_kl_divergence_2D(
        target_receptor_counts, off_target_receptor_counts
    )
...
def plot_2d_density_visualization(dataset, receptor1, receptor2, target_cells, ax):
    ...
    target_kde = KernelDensity(kernel="gaussian").fit(
        np.column_stack([receptor1_values_target, receptor2_values_target])
    )
    off_target_kde = KernelDensity(kernel="gaussian").fit(
        np.column_stack([receptor1_values_off_target, receptor2_values_off_target])
    )

    # Calculate the KL divergence using the actual receptor values
    KL_div = calculate_kl_divergence_2D(
        receptor2_values_target, receptor2_values_off_target
    )
    print(f"KL Divergence for Receptors {receptor1} & {receptor2}: {KL_div:.4f}")

    x_vals = np.linspace(
        min(receptor1_values_target.min(), receptor1_values_off_target.min()),
        max(receptor1_values_target.max(), receptor1_values_off_target.max()),
        100,
    )
    y_vals = np.linspace(
        min(receptor2_values_target.min(), receptor2_values_off_target.min()),
        max(receptor2_values_target.max(), receptor2_values_off_target.max()),
        100,
    )
...
# NOTE: SHOULD IT BE A FUNCTION OR A FIGURE?
def bindingmodel_selectivity_pair(
    dataset, target_cells, signal_receptor, special_receptor
):
    cell_types = (
        set(dataset["CellType1"])
        .union(dataset["CellType2"])
