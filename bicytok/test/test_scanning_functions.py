...
from ..scanning_funcs import (
    scan_selectivity, scan_KL_EMD
)
...
def test_selectivity_scan():
    ...
    cell_type_labels = [
        ...
        for _ in range(recAbundances.shape[0])
    ]
    targ_cells = ["A", "B", "C"]
    dose = 0.1
    sample_size = n_cells
    ...
    assert opt_affs.shape == (n_receptors, n_receptors, len(targ_cells), len(valencies[0]))
...
def test_runner_loader_roundtrip(tmp_path):
    ...
    def mock_scan_selectivity(rec_abundances, cell_type_labels, targ_cell_types, dim, valencies, **kwargs):
        n = rec_abundances.shape[1]
        k = len(targ_cell_types)
        return rng.random((n, n, k)), rng.random((n, n, k, len(valencies[0]))), rng.random((n, n, k))

    def mock_scan_KL_EMD(rec_abundances, cell_type_labels, targ_cell_types, dim, **kwargs):
        n = rec_abundances.shape[1]
        k = len(targ_cell_types)
    ...
    with patch("figures.scanning_analyses.scan_runners.scan_selectivity", side_effect=mock_scan_selectivity), \
    ...
    loaded_selec, loaded_affs, loaded_kx, loaded_recs, loaded_cts = load_selec_scan_results(selec_path)
    ...
    np.testing.assert_allclose(orig_selec, loaded_selec[np.ix_(rec_map, rec_map, ct_map)], equal_nan=True)
    np.testing.assert_allclose(orig_kx, loaded_kx[np.ix_(rec_map, rec_map, ct_map)], equal_nan=True)
    np.testing.assert_allclose(orig_affs, loaded_affs[np.ix_(rec_map, rec_map, ct_map, range(n_affs))], equal_nan=True)

    ## Testing KL/EMD scan runner and loader
    kl_emd_path = str(tmp_path / "kl_emd_scan.csv")
    with patch("figures.scanning_analyses.scan_runners.scan_KL_EMD", side_effect=mock_scan_KL_EMD), \
         patch("sys.argv", ["KL_EMD_scan", "--output-path", kl_emd_path]):
        orig_kl, orig_emd, orig_recs_kl, orig_cts_kl = run_KL_EMD_scan()
    loaded_kl, loaded_emd, loaded_recs_kl, loaded_cts_kl = load_KL_EMD_scan_results(kl_emd_path)
    rec_map_kl = [loaded_recs_kl.index(r) for r in orig_recs_kl]
    ct_map_kl = [loaded_cts_kl.index(ct) for ct in orig_cts_kl]
    np.testing.assert_allclose(orig_kl, loaded_kl[np.ix_(rec_map_kl, rec_map_kl, ct_map_kl)], equal_nan=True)
    np.testing.assert_allclose(orig_emd, loaded_emd[np.ix_(rec_map_kl, rec_map_kl, ct_map_kl)], equal_nan=True)
