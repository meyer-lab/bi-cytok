...
def model_parameters(draw):
    ...
    dose = draw(
        st.floats(
            min_value=1e-11, max_value=1e-7, allow_nan=False, allow_infinity=False
        )
    )
...
def model_affinities(draw, n_receptors=2):
    ...
    log_affinities = draw(
        arrays(
            ...
            shape=(n_receptors,),
            elements=st.floats(
                min_value=6.0, max_value=12.0, allow_nan=False, allow_infinity=False
            ),
        )
    )
...
class TestScalingInstability:
    ...
    def test_selectivity_bounds_with_scaling(
        self, scale_factor, targ_recs, off_targ_recs, params
    ):
        ...
        try:
            ...
            assert (
                SELECTIVITY_LOWER_BOUND <= selectivity <= SELECTIVITY_UPPER_BOUND
            ), f"Selectivity {selectivity:.6f} out of bounds [0, 1] with scale factor {scale_factor:.6f}"

            # Test for NaN or infinite values
            assert np.isfinite(
                selectivity
            ), f"Non-finite selectivity {selectivity} with scale factor {scale_factor:.6f}"
            assert np.all(
                np.isfinite(opt_affs)
            ), f"Non-finite affinities with scale factor {scale_factor:.6f}"
            assert np.isfinite(
                opt_kx_star
            ), f"Non-finite Kx_star with scale factor {scale_factor:.6f}"
    ...
    def test_mass_balance_conservation(
        self, scale_factor, targ_recs, off_targ_recs, params, test_affs
    ):
