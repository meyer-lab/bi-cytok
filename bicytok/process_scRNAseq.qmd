...
N_PCS = 8
sc.pp.neighbors(adata_high_variability, n_pcs=N_PCS, n_neighbors=30)
```
...
# Use the pre-defined CUSTOM_ANNOT_DIR for exports
pathlib.Path(CUSTOM_ANNOT_DIR).mkdir(parents=True, exist_ok=True)
assert os.path.isdir(CUSTOM_ANNOT_DIR)

# Validate that leiden clusters exist and align with cells
assert 'leiden' in adata_high_variability.obs.columns, "Leiden clustering not found; run clustering before exporting labels."

cell_labels_df = pd.DataFrame({
    'barcode': adata_high_variability.obs_names,
    'leiden_cluster': adata_high_variability.obs['leiden'].astype(str)
})

cell_labels_file = os.path.join(CUSTOM_ANNOT_DIR, "cell_labels.tsv")
cell_labels_df.to_csv(cell_labels_file, sep='\t', index=False)

# Basic validation to ensure exported labels align with processed AnnData
assert len(cell_labels_df) == adata_high_variability.n_obs
assert os.path.exists(cell_labels_file), f"Failed to write cell labels to {cell_labels_file}"
print(f"Exported {len(cell_labels_df)} cell labels to {cell_labels_file}")
```
