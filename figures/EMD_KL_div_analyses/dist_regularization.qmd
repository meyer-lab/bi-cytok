...
RECEPTORS_1D = [
    ...
    "CD4-1",
    "CD27",
]  # Set to None to calculate for all reasonable receptors
...
ALL_RECEPTORS = list(
    set(
        [r for pair in RECEPTOR_PAIRS_2D for r in pair]
        + [r for triad in RECEPTOR_TRIADS_3D for r in triad]
    )
)
...
_, EMD_vals = KL_EMD_1D(
    rec_abundances, on_target_mask, off_target_mask, reg_strength=None
)
...
for reg_strength in REG_STRENGTHS:
    ...
    for i, receptor in enumerate(RECEPTORS_1D):
        results_1d.append(
            {"receptor": receptor, "regularization": reg_strength, "EMD": EMD_vals[i]}
        )

    print(
        f"1D regularization {reg_strength:.3f} done in {time.time() - time_start:.2f} sec"
    )
...
for reg_strength in REG_STRENGTHS:
    ...
    print(
        f"2D regularization {reg_strength:.3f} done in {time.time() - time_start:.2f} sec"
    )
...
for reg_strength in REG_STRENGTHS:
    ...
    print(
        f"3D regularization {reg_strength:.3f} done in {time.time() - time_start:.2f} sec"
    )
