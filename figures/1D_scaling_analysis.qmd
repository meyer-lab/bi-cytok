...
# Outputs
- **8-Panel Figure**: Comprehensive analysis showing:
  ...
  7. EMD vs conversion factor (log scale)
  8. KL Divergence vs conversion factor
...
for plot_idx, config in enumerate(plot_configs):
    ...
    if plot_idx in [0, 1, 2, 6]:  # log y: binding, selectivity, EMD (not log10 params)
        ax[plot_idx].set_yscale("log")
    ax[plot_idx].set_xlabel("Conversion Factor", fontsize=15)
    ax[plot_idx].set_ylabel(config["ylabel"], fontsize=15)
    ax[plot_idx].set_title(config["title"], fontsize=15)

    # Add legend for specific plots
    if plot_idx in [1, 2, 3, 4, 5, 6, 7]:
        legend_title = (
