...
TARGET_COLOR = "#CF4D6F"
off_targ_palette = cm.get_cmap("tab10", len(off_targ_cell_types))
...
for i, (abundances, color, label) in enumerate(
    zip(all_cell_abundances, colors, labels)
):
    if len(abundances) == 0:
        continue
    sns.histplot(
...
for cell_type, color, label in zip(populations, colors, labels):
...
if len(off_targ_df) > 10:
    ...
    sns.histplot(
        ...
        bins=30,
        kde=True,
        kde_kws={"bw_method": "scott", "linestyle": "--", "linewidth": 2.5},
        ax=ax_top,
    )
    sns.histplot(
