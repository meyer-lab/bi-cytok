...
signal = ["CD122"]  # Could be "Prototype_Signal_Receptor"
cell_type = "Treg"
...
if receptors is None:
    receptors = [col for col in CITE_DF.columns if col not in signal]

if include_prototype_signal_receptor:
    rng = np.random.default_rng()
    prototype_signal_receptor = rng.normal(loc=50, scale=5, size=(CITE_DF.shape[0],))
    CITE_DF.insert(0, "Prototype_Signal_Receptor", prototype_signal_receptor)
...
print(
    f"Number of different cell types in sample: {len(sample_df['Cell Type'].unique())}"
)
