...
# Color every point by the gain of the scan named in that scan's "gain_ref": its own gain, or
# another scan's gain mapped onto these coordinates by pair key. Pointing the tight scan at the
# loose scan shows where the loose-bound-synergistic pairs land once Kx* is capped (unmatched
# pairs get NaN -> gray).
gain_by_scan_key = {
    lbl: pd.Series(df["log2_gain"].to_numpy(), index=pair_key(df))
    for lbl, df in pair_dfs.items()
}
for lbl, series in gain_by_scan_key.items():
    assert series.index.is_unique, f"Duplicate pair keys in scan {lbl!r}."
scan_colors_gain = {}
for scan in SCANS:
    ref_gain = gain_by_scan_key[scan["gain_ref"]]
    scan_colors_gain[scan["label"]] = ref_gain.reindex(
        pair_key(pair_dfs[scan["label"]])
    ).to_numpy()
...
fig.colorbar(
    ...
    label="log2 selectivity gain, reference scan  (gray = none)", fraction=0.03, pad=0.02,
)
plt.show()
```
