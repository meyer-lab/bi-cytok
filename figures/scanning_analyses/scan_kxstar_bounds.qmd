...
A consolidated scatter grid (`fig-scatter-grid`) gathers every per-pair scatter into one figure:
rows are comparisons, columns are scans (loose bound left, tight bound right). The first four
rows plot each output (selectivity, the three affinities) against optimal log10(Kx*); the last
three plot log2 selectivity gain against pair selectivity and against receptor affinity (signal,
and pooled target arms). Every panel shares one coloring scheme — all pairs in gray, with pairs
that are synergistic *under the loose bound* overlaid and colored by their loose-bound log2 gain.
Because that loose-bound gain is mapped onto each scan's own coordinates by pair key, the
tight-bound column shows exactly where the once-synergistic pairs are trapped when Kx* is capped:
pinned at the wall, driven to the affinity ceiling, and with their gain collapsed to ~0. This one
figure therefore answers whether the high-Kx* pairs are the selective, synergistic ones; whether
that population is dominated by degenerate near-floor pairs; how affinity compensates for capped
Kx*; and where the constrained pairs end up. Points are rasterized (~276k per panel).

`fig-scatter-grid-celltype` repeats the same grid with points colored by cell type instead of
gain, exposing which cell types occupy which regions of these spaces.
...
- **`fig-scatter-grid`**: rasterized scatter grid consolidating every per-pair comparison
  (rows) across scans (columns) — each output vs. optimal log10(Kx*), and log2 gain vs.
  selectivity and affinity — with all panels colored by each pair's loose-bound log2 gain,
  mapped onto every scan so the tight-bound column shows where synergistic pairs are trapped
- **`fig-scatter-grid-celltype`**: the same grid colored by cell type, showing where cell
  types sit in these spaces
...
- Printed summaries: Kx* pair count / median / fraction pinned at each bound; selectivity floor
  and high-tail fractions; synergistic and high-gain fractions; degeneracy (floor share) and
  compensation (target-arm affinity) of the synergistic tail; and the fate of the
  loose-bound-synergistic pairs under the tight bound (gain collapse and Kx*-pinned fraction)
...
# UNDER THE LOOSE BOUND overlaid and colored by their loose-bound log2 gain (strongest drawn
# last). Because that color is the loose-bound gain mapped onto each scan's own coordinates,
# the tight-bound column shows exactly where the once-synergistic pairs are trapped once Kx* is
# capped — pinned at the wall, driven to the affinity ceiling, gain collapsed to ~0. Points are
# rasterized (~276k per panel). Axis scales are shared within a row (across scans), not down a
# column, so x and y labels are set per row rather than shared globally.

from matplotlib.colors import Normalize

SELEC_FLOOR = 0.505       # pair selectivity at/below this is effectively no selectivity
STRONG_LOG2_GAIN = 0.05   # log2 gain above which loose-bound synergy is non-trivial
...
# One dict per comparison (grid row). xrefs/yrefs are gray reference lines; kx_vline draws each
# scan's Kx* upper bound; ylim fixes the display range (affinity panels use padded bounds).
COMPARISONS = [
    dict(get=_vs_kxstar("sel"), xlabel=KX_XLABEL, ylabel="Selectivity",
         ylim=None, xrefs=(), yrefs=(0.5,), kx_vline=True),
    dict(get=_vs_kxstar("aff0"), xlabel=KX_XLABEL, ylabel="Signal aff, log10(M)",
         ylim=AFFINITY_BOUNDS, xrefs=(), yrefs=(), kx_vline=True),
    dict(get=_vs_kxstar("aff1"), xlabel=KX_XLABEL, ylabel="Target 1 aff, log10(M)",
         ylim=AFFINITY_BOUNDS, xrefs=(), yrefs=(), kx_vline=True),
    dict(get=_vs_kxstar("aff2"), xlabel=KX_XLABEL, ylabel="Target 2 aff, log10(M)",
         ylim=AFFINITY_BOUNDS, xrefs=(), yrefs=(), kx_vline=True),
    dict(get=_gain_vs("sel"), xlabel="Pair selectivity", ylabel=GAIN_YLABEL,
         ylim=None, xrefs=(SELEC_FLOOR,), yrefs=(0.0,), kx_vline=False),
    dict(get=_gain_vs("aff0"), xlabel="Signal receptor affinity, log10(M)", ylabel=GAIN_YLABEL,
         ylim=None, xrefs=(), yrefs=(0.0,), kx_vline=False),
    dict(get=_gain_vs("targ_aff"), xlabel="Target receptor affinity, log10(M)",
         ylabel=GAIN_YLABEL, ylim=None, xrefs=(), yrefs=(0.0,), kx_vline=False),
]
...
def scan_base_arrays(scan):
    ...
    return {
        "kx": df["Kx_star"].to_numpy(), "sel": df["Selectivity"].to_numpy(),
...
def build_comparison_grid(scan_colors, draw_points):
    ...
    draw_points: fn(ax, x, y, color_values, scan) -> optional mappable (captured for a colorbar).
    """
    fig, axes = plt.subplots(
        len(COMPARISONS), len(SCANS),
    ...
    for c, scan in enumerate(SCANS):
        ...
        for r, comp in enumerate(COMPARISONS):
            ...
            drawn = draw_points(ax, x, y, cvals, scan)
            ...
            if comp["kx_vline"] and scan["kx_upper"] is not None:
                ax.axvline(scan["kx_upper"], color="0.3", ls="--", lw=1.2, zorder=3)
    ...
    for r in range(len(COMPARISONS)):
        _sync_row(list(axes[r]))
    return fig, axes, mappable


# Color every point by the gain the pair reaches under the loosest bound, mapped across scans by
# pair key. For the loose scan this is its own gain; for tighter scans it is the loose-bound gain
# transferred onto that same pair's constrained coordinates (unmatched pairs get NaN -> gray).
loose_scan = SCANS[0]
loose_gain = pd.Series(
    pair_dfs[loose_scan["label"]]["log2_gain"].to_numpy(),
    index=pair_key(pair_dfs[loose_scan["label"]]),
)
assert loose_gain.index.is_unique, "Duplicate pair keys in the loose-bound scan."
scan_colors_gain = {
    scan["label"]: loose_gain.reindex(pair_key(pair_dfs[scan["label"]])).to_numpy()
    for scan in SCANS
}
...
def draw_gain_points(ax, x, y, cvals, scan):
    """Gray context cloud plus a Reds overlay of the loose-bound-synergistic pairs."""
    ax.scatter(x, y, s=3, alpha=0.4, color="0.82", linewidths=0, rasterized=True, zorder=1)
    ...
    return ax.scatter(
        x[synergistic][order], y[synergistic][order], c=cvals[synergistic][order],
        cmap="Reds", norm=gain_norm, s=5, alpha=0.9, linewidths=0, rasterized=True, zorder=2,
    )
...
fig.colorbar(
    ...
    label="log2 selectivity gain, loose bound  (gray = none)", fraction=0.03, pad=0.02,
)
plt.show()

# ── Degeneracy check: are the loose-synergistic pairs floored ratio artifacts? ─
print(
...
for scan in SCANS:
    ...
    tail = syn.query("log2_gain > @STRONG_LOG2_GAIN")
    tail_med = tail["Selectivity"].median() if len(tail) else np.nan
    print(
        f"  {scan['label']:26s}  n_syn={len(syn):6d}  "
        f"frac at floor={np.mean(syn['Selectivity'] < SELEC_FLOOR):.3f}  |  "
        f"tail n={len(tail):5d}  median pair-selec={tail_med:.3f}"
    )

# ── Compensation: target-arm affinity of the non-trivial tail per scan ─────────
...
for scan in SCANS:
    ...
    ceil = np.mean(targ_aff > AFFINITY_OPT_BOUNDS[1] - AFF_CEIL_EPS) if len(targ_aff) else np.nan
    print(
        f"  {scan['label']:26s}  median target aff={med:.2f}  "
        f"frac at ceiling ({AFFINITY_OPT_BOUNDS[1]:.0f})={ceil:.3f}"
    )

# ── Fate: where the loose-bound-synergistic pairs end up under the tight bound ─
tight_scan = SCANS[-1]
loose_pairs, tight_pairs = pair_dfs[loose_scan["label"]], pair_dfs[tight_scan["label"]]
fate = pd.DataFrame(
    {"key": pair_key(loose_pairs), "loose_gain": loose_pairs["log2_gain"].to_numpy()}
).merge(
    pd.DataFrame({
        "key": pair_key(tight_pairs),
        "tight_gain": tight_pairs["log2_gain"].to_numpy(),
        "tight_pinned": (
            np.abs(tight_pairs["Kx_star"] - tight_scan["kx_upper"]) < PIN_TOL
        ).to_numpy(),
    }),
    on="key",
)
strong = fate[fate["loose_gain"] > STRONG_LOG2_GAIN]
print(
    f"\nFate of loose-bound-synergistic pairs (log2 gain > {STRONG_LOG2_GAIN}, "
    f"n={len(strong)} matched) under the tight bound:"
)
print(
    f"  median log2 gain {strong['loose_gain'].median():+.3f} (loose) -> "
    f"{strong['tight_gain'].median():+.3f} (tight)   "
    f"Kx*-pinned={strong['tight_pinned'].mean():.3f}   "
    f"still > {STRONG_LOG2_GAIN}={np.mean(strong['tight_gain'] > STRONG_LOG2_GAIN):.3f}"
)
```
...
fig.legend(
    handles=[Patch(facecolor=CT_COLORS[i], label=CELL_TYPES[i]) for i in range(len(CELL_TYPES))],
    loc="center left", bbox_to_anchor=(1.0, 0.5), ncol=1, fontsize=FS_LEGEND - 2,
...
def classify_copinning(df, kx_upper):
    ...
    return pd.DataFrame(
        {"key": pair_key(df), "cat": 2 * kx_pinned.astype(int) + aff_ceiling.astype(int)}
    )
