...
def build_comparison(metric_1, metric_2, cell_types, axis_norm):
    ...
    if axis_norm == "z-score":
        all_df["s1"] = (all_df["s1"] - all_df["s1"].mean()) / (all_df["s1"].std() + 1e-9)
        all_df["s2"] = (all_df["s2"] - all_df["s2"].mean()) / (all_df["s2"].std() + 1e-9)
    elif axis_norm == "rank":
...
def plot_comparison(key):
    ...
    # Spearman correlation per cell type and overall.
    rows = []
    for ct in TARG_CELL_TYPES:
...
#| fig-cap: "KL divergence vs. EMD across target cell types. Each point is a receptor pair for a given cell type; the top pair per axis is starred."
plot_comparison("kl_emd")
```

```{python}
#| label: fig-scan-comp-kl-selec
#| fig-cap: "KL divergence vs. binding-model selectivity across target cell types. Each point is a receptor pair for a given cell type; the top pair per axis is starred."
plot_comparison("kl_selec")
```

```{python}
#| label: fig-scan-comp-emd-selec
#| fig-cap: "EMD vs. binding-model selectivity across target cell types. Each point is a receptor pair for a given cell type; the top pair per axis is starred."
