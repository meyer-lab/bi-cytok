...
off_targ_cell_types = ["CD4 TCM", "CD8 TCM", "CD4 TEM", "CD8 TEM", "ILC"]
# off_targ_cell_nums = {
#     "CD4 TCM": 0.3,
#     "CD8 TCM": 0.3,
#     "CD4 TEM": 0.15,
...
for sample_size in sample_sizes:
    ...
    for i in range(randomizations):
        ...
        if off_targ_cell_nums is not None:
            sampled_off_target_cells = pd.concat(
                [
                    sample_df[
                        sample_df["Cell Type"] == cell_type
                    ].sample(
                        min(int(count * sample_size), sample_df[sample_df["Cell Type"] == cell_type].shape[0]),
                        replace=True,
                    )
                    for cell_type, count in off_targ_cell_nums.items()
                ]
            )
        else:
