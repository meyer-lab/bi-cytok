...
# Summary
Performs bootstrap analysis to quantify variability in KL Divergence, EMD, selectivity metrics, and off-target cell type proportions across different sample sizes. Generates box plots showing the distribution of metric values and reports average computation times for model optimization and distance metric calculations.
...
- **Box Plot 5**: Bootstrap distribution of off-target cell type proportions across sample sizes
...
for sample_size in sample_sizes:
    ...
    for i in range(randomizations):
        ...
        for ct in off_targ_cts:
            if ct in sampled_off_target_cells["Cell Type"].unique():
                off_targ_props.append(
                    sampled_off_target_cells[sampled_off_target_cells["Cell Type"] == ct].shape[0]
                    / sample_size
                )
            else:
                off_targ_props.append(0)
        ...
        metrics.append(
            {
                ...
                "off_target_props": off_targ_props,
            }
        )
...
plt.xlabel("Sample Size", fontsize=15)
plt.ylabel("Affinity Value", fontsize=15)
plt.legend(title="Affinity", fontsize=15)
plt.show()
```
...
off_targ_props_df = pd.DataFrame(
    metrics_df["off_target_props"].tolist(), columns=[f"{ct}" for ct in off_targ_cts]
)
off_targ_props_df["sample_size"] = metrics_df["sample_size"].astype(str)
