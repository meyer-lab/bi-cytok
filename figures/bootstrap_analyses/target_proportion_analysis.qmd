...
for target_prop in TARGET_PROPORTIONS:
    ...
    for i in range(RANDOMIZATIONS):
        sampled_target_cells = all_target_cells.sample(
            min(num_target_cells, all_target_cells.shape[0]), replace=True
        )
        sampled_off_target_cells = all_off_target_cells.sample(
            min(num_off_target_cells, all_off_target_cells.shape[0]), replace=True
        )
        ...
        print(
            f"Target Proportion: {target_prop:.2f}, Randomization: {i + 1}/{RANDOMIZATIONS}, Model Time: {time_ellapsed_mod:.4f}s"
        )
        metrics.append(
            {
...
```{python}
#| label: fig-targ-prop-kldiv
import matplotlib.pyplot as plt
import seaborn as sns

from bicytok.figure_styles import COLORS, FIGSIZE, categorical_colors, set_tick_count

fig, ax = plt.subplots(figsize=FIGSIZE["single_panel"])
sns.boxplot(x="target_proportion", y="KL_div", data=metrics_df, color=COLORS[TARG_CELL], ax=ax)
ax.set_xlabel("Target Cell Proportion")
...
```{python}
#| label: fig-targ-prop-emd

fig, ax = plt.subplots(figsize=FIGSIZE["single_panel"])
sns.boxplot(x="target_proportion", y="EMD", data=metrics_df, color=COLORS[TARG_CELL], ax=ax)
ax.set_xlabel("Target Cell Proportion")
ax.set_ylabel("EMD")
set_tick_count(ax) 
plt.tight_layout()
...
```{python}
#| label: fig-targ-prop-selec

fig, ax = plt.subplots(figsize=FIGSIZE["single_panel"])
sns.boxplot(x="target_proportion", y="selectivity", data=metrics_df, color=COLORS[TARG_CELL], ax=ax)
set_tick_count(ax)
