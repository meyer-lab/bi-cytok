...
dose = 1e-10
valency_main = np.array([[2, 1, 1]])
valency_diag = np.array([[2, 2]])
...
if receptors is None:
    receptors = [
        col
        for col in epitopes_df.columns
        if col != "Cell Type" and col not in signal
    ]
...
for i, j in zip(row, col, strict=False):
    ...
    if rec1 == rec2:
        valency = valency_diag
        rec_abun = sample_df[[rec1]].to_numpy()
        receptor_abuns = np.hstack((signal_abun, rec_abun))
    else:
        valency = valency_main
    ...
    opt_selec, opt_affs, opt_Kx_star = optimize_affs(targ_abun, off_targ_abun, dose, valencies=valency)
...
sns.heatmap(
    selec_df, cmap="bwr", cbar=True, xticklabels=True, yticklabels=True, vmin=0.7, vmax=1.0
)
