...
for receptor in epitopes:
    ...
    for valency in test_valencies:
        ...
        time_end = time.time()
        print(
            f"Processing time for {receptor} (Valency {valency}): {time_end - time_start} seconds"
        )
...
selectivity_per_receptor = (
    metrics_df.dropna(subset=["Selectivity"])
    .groupby("Receptor Pair")["Selectivity"]
    .max()
    .reset_index()
)
...
metrics_df_selectivity_only_kl = metrics_df[
    metrics_df["Receptor Pair"].isin(selectivity_only_kl)
]
...
metrics_df_selectivity_only_emd = metrics_df[
    metrics_df["Receptor Pair"].isin(selectivity_only_emd)
]
```
...
if len(metrics_df_kl_both) > 0:
    for valency in metrics_df_kl_both["Valency"].unique():
        ...
        ax.scatter(
            ...
            if valency == metrics_df_kl_both["Valency"].unique()[0]
            else "",
        )
...
if len(metrics_df_kl_only) > 0:
    for valency in metrics_df_kl_only["Valency"].unique():
        ...
        ax.scatter(
            ...
            marker=marker,
            label="KL Only"
            if valency == metrics_df_kl_only["Valency"].unique()[0]
            else "",
        )
...
if len(metrics_df_selectivity_only_kl) > 0:
    for valency in metrics_df_selectivity_only_kl["Valency"].unique():
        subset = metrics_df_selectivity_only_kl[
            metrics_df_selectivity_only_kl["Valency"] == valency
        ]
        marker = "o" if valency == "Valency 2" else "X"
        ax.scatter(
            subset["KL Divergence"],
            ...
            if valency == metrics_df_selectivity_only_kl["Valency"].unique()[0]
            else "",
        )
...
if len(metrics_df_emd_both) > 0:
    for valency in metrics_df_emd_both["Valency"].unique():
        ...
        ax.scatter(
            ...
            if valency == metrics_df_emd_both["Valency"].unique()[0]
            else "",
        )
...
if len(metrics_df_emd_only) > 0:
    for valency in metrics_df_emd_only["Valency"].unique():
        ...
        ax.scatter(
            ...
            marker=marker,
            label="EMD Only"
            if valency == metrics_df_emd_only["Valency"].unique()[0]
            else "",
        )
...
if len(metrics_df_selectivity_only_emd) > 0:
    for valency in metrics_df_selectivity_only_emd["Valency"].unique():
        subset = metrics_df_selectivity_only_emd[
            metrics_df_selectivity_only_emd["Valency"] == valency
        ]
        marker = "o" if valency == "Valency 2" else "X"
        ax.scatter(
            subset["EMD"],
            ...
            marker=marker,
            label="Selectivity Only"
            if valency == metrics_df_selectivity_only_emd["Valency"].unique()[0]
            else "",
        )
