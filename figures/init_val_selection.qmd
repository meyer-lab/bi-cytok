...
for init_aff in initial_affinities:
    ...
    for rec1, rec2 in receptor_combinations:
        ...
        init_vals = np.array(
            [fixed_affinity, fixed_affinity, fixed_affinity, fixed_Kx_star]
        )
...
optimize_affs(
    ...
    valencies=valencies,
    init_vals=np.array(
        [fixed_affinity[0], fixed_affinity[1], fixed_affinity[2], initial_kx_stars[0]]
    ),
)
print("Warmup complete.")
...
for init_kx in initial_kx_stars:
    ...
    for rec1, rec2 in receptor_combinations:
        ...
        receptor_abuns = np.hstack((signal_abun, rec1_abun, rec2_abun))
        init_vals = np.array(
            [fixed_affinity[0], fixed_affinity[1], fixed_affinity[2], init_kx]
        )
...
plt.show()
sns.violinplot(
    data=metrics_df[["Affinity_Signal", "Affinity_Target_Rec1", "Affinity_Target_Rec2"]]
)
plt.show()
...
# Check initialization search method
selec_init, affs_init, Kx_init, opt_init_params, _ = optimize_affs(
    targ_abun,
    off_targ_abun,
    dose,
    valencies=valencies,
...
# Calculate the optimal selectivity value using multiple random starting points
...
for sig_aff in signal_aff_range:
    for targ_aff in target_aff_range:
        for Kx in Kx_star_range:
            ...
            print(init_vals)
            print(isinstance(init_vals, str))
...
scan_df["wrong_optimum"] = (
    scan_df["selectivity_error"] > 0.05 * optimal_selectivity
) & (~scan_df["exception_raised"])
scan_df["failed"] = scan_df["exception_raised"] | scan_df["wrong_optimum"]

print("\nScan complete!")
print(f"Total runs: {len(scan_df)}")
print(
    f"Successful optimizations: {(~scan_df['failed']).sum()} ({100 * (~scan_df['failed']).sum() / len(scan_df):.1f}%)"
)
print(
    f"Exception raised (true model failure): {scan_df['exception_raised'].sum()} ({100 * scan_df['exception_raised'].sum() / len(scan_df):.1f}%)"
)
print(
    f"Wrong optimum (>5% error): {scan_df['wrong_optimum'].sum()} ({100 * scan_df['wrong_optimum'].sum() / len(scan_df):.1f}%)"
)
```
...
for _, row in failed_cases.iterrows():
    print(
        f"Init Signal Aff: {row['signal_aff_init']}, Init Target Aff: {row['target_aff_init']}, Init Kx_star: {row['Kx_star_init']}"
    )
...
ax1.scatter(
    succeeded["signal_aff_init"],
    succeeded["target_aff_init"],
    succeeded["Kx_star_init"],
    c="green",
    marker="o",
    alpha=0.6,
    s=20,
...
ax1.scatter(
    wrong_optimum["signal_aff_init"],
    wrong_optimum["target_aff_init"],
    wrong_optimum["Kx_star_init"],
    c="orange",
    marker="s",
    alpha=0.7,
    s=30,
...
ax1.scatter(
    exception_failures["signal_aff_init"],
    exception_failures["target_aff_init"],
    exception_failures["Kx_star_init"],
    c="red",
    marker="x",
    alpha=0.8,
    s=50,
    label="Exception (Model Failure)",
)
ax1.scatter(
    opt_init_params[0],
    opt_init_params[1],
    ...
    edgecolors="black",
    linewidths=1.5,
    label="Search Method Optimal Start",
)
...
pivot_sig_targ = (
    scan_df.groupby(["signal_aff_init", "target_aff_init"])["failed"]
    .mean()
    .reset_index()
)
pivot_matrix = pivot_sig_targ.pivot(
    index="target_aff_init", columns="signal_aff_init", values="failed"
)
...
pivot_targ_kx = (
    scan_df.groupby(["target_aff_init", "Kx_star_init"])["failed"].mean().reset_index()
)
pivot_matrix2 = pivot_targ_kx.pivot(
    index="Kx_star_init", columns="target_aff_init", values="failed"
)
...
axes[0].hist(
    scan_df[~scan_df["failed"]]["selectivity_error"], bins=30, edgecolor="black"
)
axes[0].set_xlabel("Selectivity Error")
axes[0].set_ylabel("Count")
axes[0].set_title("Distribution of Selectivity Error (Successful Runs)")
axes[0].axvline(
    0.01 * optimal_selectivity, color="red", linestyle="--", label="1% threshold"
)
axes[0].legend()
...
axes[1].scatter(
    successful["signal_aff_init"],
    successful["selectivity"],
    alpha=0.5,
    label="vs Signal Aff",
    s=10,
)
axes[1].scatter(
    successful["target_aff_init"],
    successful["selectivity"],
    alpha=0.5,
    label="vs Target Aff",
    s=10,
...
print(
    f"Average selectivity of failures: {scan_df[scan_df['failed']]['selectivity'].mean():.4f}"
)
print(
    f"Standard deviation of selectivity of failures: {scan_df[scan_df['failed']]['selectivity'].std():.4f}"
)
```

```{python}
# Characterize objective function landscape within affinity/Kx_star space
import numpy as np
import pandas as pd

from bicytok.imports import importCITE, sample_receptor_abundances
from bicytok.selectivity_funcs import compute_selectivity

receptors = ["CD45RA", "CD338"]
signal = ["CD122"]
cell_type = "Treg"
dose = 1e-10
valencies = np.array([[2, 1, 1]])
cell_categorization = "CellType2"
sample_size = 1000
n = 12  # Number of points per parameter to scan
...
for sig_aff in signal_aff_range:
    for targ_aff in target_aff_range:
        for Kx in Kx_star_range:
            ...
            loss = compute_selectivity(
                targ_abun,
                off_targ_abun,
                affinities=[sig_aff, targ_aff, targ_aff],
                dose=dose,
                valencies=valencies,
                Kx_star=Kx,
                method="mean",
            ...
            scan_results.append(
                {
                    "signal_affinity": sig_aff,
                    "target_affinity": targ_aff,
                    "Kx_star": Kx,
                    "selectivity": 1 / loss,
                }
            )

landscape_df = pd.DataFrame(scan_results)
print(landscape_df)
```

```{python}
#| fig-cap: "Selectivity landscape across affinity and Kx_star space"
import matplotlib.pyplot as plt
import seaborn as sns

# Plot affinity landscapes across Kx_star slices
fig, axes = plt.subplots(1, n, figsize=(4 * n, 4), sharey=True)
for i, Kx in enumerate(np.linspace(-15, -9, n)):
    ax = axes[i]
    subset = landscape_df[landscape_df["Kx_star"] == Kx]
    pivot_matrix = subset.pivot(
        index="target_affinity", columns="signal_affinity", values="selectivity"
    )
    sns.heatmap(pivot_matrix, ax=ax, cmap="viridis")
    ax.set_title(f"log10(Kx_star) = {Kx:.11f}")
    ax.set_xlabel("Signal Affinity")
    if i == 0:
        ax.set_ylabel("Target Affinity")
    else:
        ax.set_ylabel("")
plt.suptitle("Selectivity Landscape Across Affinity and Kx_star Space")
```
...
```{python}
# Sweep across receptor pairs with fixed initialization to identify suboptimal failures
import numpy as np
import pandas as pd
from itertools import combinations_with_replacement
...
# Ill-conditioned starting point
ill_conditioned_init = np.array([12.0, 12.0, 12.0, -15.0])
well_conditioned_init = np.array([6.0, 7.0, 7.0, -9.0])
...
sample_df = sample_receptor_abundances(epitopes_df, sample_size, cell_type)
filtered_df = filter_receptor_abundances(sample_df, targ_cell_type=cell_type, whitelist=signal + receptors)
receptors = filtered_df.columns.tolist()
receptors.remove("Cell Type")

targ_mask = (filtered_df["Cell Type"] == cell_type).to_numpy()
off_targ_mask = ~targ_mask
signal_abun = filtered_df[signal].to_numpy()

receptor_combinations = list(combinations_with_replacement(receptors, 2))

# Store results for both initialization conditions
results = []

print(f"Testing {len(receptor_combinations)} receptor pairs...\n")

for rec1, rec2 in receptor_combinations:
    print(f"Testing receptors {rec1}, {rec2}")
    
    rec1_abun = filtered_df[[rec1]].to_numpy()
    rec2_abun = filtered_df[[rec2]].to_numpy()
    receptor_abuns = np.hstack((signal_abun, rec1_abun, rec2_abun))
    
    targ_abun = receptor_abuns[targ_mask]
    off_targ_abun = receptor_abuns[off_targ_mask]
    
    # Test with well-conditioned initialization
    try:
        selec_good, affs_good, Kx_good = optimize_affs(
            targ_abun,
            off_targ_abun,
            dose,
            valencies=valencies,
            init_vals=well_conditioned_init,
        )
        selectivity_good = 1 / selec_good
    except Exception as e:
        print(f"  Well-conditioned failed with exception: {e}")
        selectivity_good = np.nan
        affs_good = np.full(3, np.nan)
        Kx_good = np.nan
    
    # Test with ill-conditioned initialization
    try:
        selec_ill, affs_ill, Kx_ill = optimize_affs(
            targ_abun,
            off_targ_abun,
            dose,
            valencies=valencies,
            init_vals=ill_conditioned_init,
        )
        selectivity_ill = 1 / selec_ill
    except Exception as e:
        print(f"  Ill-conditioned failed with exception: {e}")
        selectivity_ill = np.nan
        affs_ill = np.full(3, np.nan)
        Kx_ill = np.nan
    
    results.append({
        "receptor_1": rec1,
        "receptor_2": rec2,
        "selectivity_good_init": selectivity_good,
        "selectivity_ill_init": selectivity_ill,
        "affs_good_init": affs_good,
        "affs_ill_init": affs_ill,
        "Kx_good_init": Kx_good,
        "Kx_ill_init": Kx_ill,
    })

results_df = pd.DataFrame(results)

# Identify failures (where ill-conditioned init gives significantly worse or NaN selectivity)
results_df["selectivity_diff"] = (
    results_df["selectivity_ill_init"] - results_df["selectivity_good_init"]
)
results_df["relative_diff"] = (
    np.abs(results_df["selectivity_diff"]) / results_df["selectivity_good_init"]
)

# Define failure as <0.5% difference or NaN
results_df["suboptimal_failure"] = (
    (results_df["relative_diff"] < 0.005) | 
    (results_df["selectivity_ill_init"].isna())
)

failed_pairs = results_df[results_df["suboptimal_failure"]]

print("\n" + "="*80)
print("SUMMARY")
print("="*80)
print(f"Total receptor pairs tested: {len(results_df)}")
print(f"Pairs with suboptimal failures: {len(failed_pairs)} ({100 * len(failed_pairs) / len(results_df):.1f}%)")
print("\nFailed receptor pairs:")
for _, row in failed_pairs.iterrows():
    print(f"  {row['receptor_1']}, {row['receptor_2']}: "
          f"Good init = {row['selectivity_good_init']:.4f}, "
          f"Ill init = {row['selectivity_ill_init']:.4f}, "
          f"Diff = {row['relative_diff']:.4f}")
```

```{python}
#| fig-cap: "Receptor abundance distributions for failed receptor pairs"
import matplotlib.pyplot as plt

# Generate histograms for each failed receptor pair
N_FAILED = len(failed_pairs)

if N_FAILED > 0:
    fig, axes = plt.subplots(N_FAILED, 2, figsize=(12, 4 * N_FAILED))
    
    # Ensure axes is 2D even with single failure
    if N_FAILED == 1:
        axes = axes.reshape(1, -1)
    
    for idx, (_, row) in enumerate(failed_pairs.iterrows()):
        rec1 = row["receptor_1"]
        rec2 = row["receptor_2"]
        
        # Get receptor abundances
        rec1_abun = sample_df[[rec1]].to_numpy().flatten()
        rec2_abun = sample_df[[rec2]].to_numpy().flatten()
        
        # Split by target and off-target
        rec1_targ = rec1_abun[targ_mask]
        rec1_off_targ = rec1_abun[off_targ_mask]
        rec2_targ = rec2_abun[targ_mask]
        rec2_off_targ = rec2_abun[off_targ_mask]
        
        # Plot receptor 1 distributions
        ax1 = axes[idx, 0]
        ax1.hist(rec1_targ, bins=30, alpha=0.6, label=f"{cell_type} (target)", 
                 color="blue", edgecolor="black")
        ax1.hist(rec1_off_targ, bins=30, alpha=0.6, label="Off-target", 
                 color="red", edgecolor="black")
        ax1.set_xlabel(f"{rec1} Abundance")
        ax1.set_ylabel("Count")
        ax1.set_title(f"{rec1} Distribution")
        ax1.legend()
        # ax1.set_yscale("log")
        
        # Plot receptor 2 distributions
        ax2 = axes[idx, 1]
        ax2.hist(rec2_targ, bins=30, alpha=0.6, label=f"{cell_type} (target)", 
                 color="blue", edgecolor="black")
        ax2.hist(rec2_off_targ, bins=30, alpha=0.6, label="Off-target", 
                 color="red", edgecolor="black")
        ax2.set_xlabel(f"{rec2} Abundance")
        ax2.set_ylabel("Count")
        ax2.set_title(f"{rec2} Distribution")
        ax2.legend()
        # ax2.set_yscale("log")
        
        # Add suptitle for this pair
        fig.text(0.5, 1.0 - (idx / N_FAILED) + 0.02, 
                 f"Failed Pair: {rec1} + {rec2}", 
                 ha="center", va="top", fontsize=12, fontweight="bold")
    
    plt.tight_layout()
    plt.show()
else:
    print("No failed receptor pairs to display.")
```