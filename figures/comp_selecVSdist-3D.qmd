...
model_valencies_main = np.array([[2, 1, 1]])
model_valencies_diag = np.array([[2, 2]])
...
if receptors is not None:
    ...
    receptors = [col for col in filtered_sample_df.columns if col != "Cell Type"]
    receptor_pairs = generate_receptor_pairs(receptors)
...
for receptor_pair in receptor_pairs:
    ...
    EMD_vals.append(EMD)
    print(
        f"Metric calculation time for {receptor_pair}: {time.time() - time_start_metric} seconds"
    )
    ...
    print(
        f"Optimization time for {receptor_pair}: {time.time() - time_start_opt} seconds"
    )
